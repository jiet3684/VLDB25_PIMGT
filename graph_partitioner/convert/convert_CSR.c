...
int main(int argc, char** argv) {
    ...
    while (fgets(line, 200, fp) != NULL) {
        ...
        if (edges[offset].from == edges[offset].to) continue;

        offset++;
    }
    mat.ne = offset;

    // int *offsets = calloc(mat.nr, 4);
    // for (int e = 0; e < mat.ne; ++e) {
    //     mat.nnz[edges[e].from]++;
    // }
    // mat.ptr[0] = 0;
    // for (int n = 0; n < mat.nr; ++n) {
    //     mat.ptr[n + 1] = mat.ptr[n] + mat.nnz[n];
    // }
    // for (int e = 0; e < mat.ne; ++e) {
    //     int node = edges[e].from;
    //     mat.idx[mat.ptr[node] + offsets[node]++] = edges[e].to;
    // }
    ...
    printf("Input: %s\n\tNumber of Rows:\t\t%d\n\tNumber of Columns:\t%d\n\tNumber of Edges:\t%d (%d)\n\n", input_File, mat.nr - 1, mat.nc - 1, mat.ne, mat.inc_ptr[mat.nr]);
    char input[64];
    strcpy(input, input_File);
    strcat(input, ".csr");
    fp = fopen(input, "wb");
    // int row = mat.nr - 1, col = mat.nc - 1, edge = mat.ne;
    // fwrite(&row, sizeof(int), 1, fp);
    // fwrite(&col, sizeof(int), 1, fp);
    // fwrite(&edge, sizeof(int), 1, fp);

    // fwrite(mat.nnz + 1, sizeof(int), row, fp);
    // fwrite(mat.ptr + 1, sizeof(int), row + 1, fp);
    // fwrite(mat.idx, sizeof(int), edge, fp);
    // fwrite(mat.inc_nnz + 1, sizeof(int), row, fp);
    // fwrite(mat.inc_ptr + 1, sizeof(int), row + 1, fp);
    // fwrite(mat.inc_idx, sizeof(int), mat.inc_ptr[mat.nr], fp);
    ...
    // for (int i = 0; i < row; ++i) {
    //     printf("%d:\n", i);
    //     for (int e = mat.inc_ptr[i]; e < mat.inc_ptr[i + 1]; ++e) {
    //         printf("\t%d\n", mat.inc_idx[e]);
    //     }
    // }
